...
describe('isEqualErr()', () => {
    it('returns true for the same Error object instance', () => {
        ...
        assert.strictEqual(isEqualErr(err, err), true)
    })
    ...
    it('returns true for two Error objects with the same name and message', () => {
        ...
        assert.strictEqual(isEqualErr(err1, err2), true)
    })
    ...
    it('returns false for two Error objects with different messages', () => {
        ...
        assert.strictEqual(isEqualErr(err1, err2), false)
    })
    ...
    it('returns false when one value is not an Error', () => {
        const err = new Error('test')
        assert.strictEqual(isEqualErr('test', err), false)
        assert.strictEqual(isEqualErr(null, err), false)
        assert.strictEqual(isEqualErr({}, err), false)
    })
    ...
    it('does not compare stack traces', () => {
        ...
        assert.strictEqual(isEqualErr(err1, err2), true) // But they are considered the same
    })

    it('throws a TypeError if the reference value is not an Error', () => {
        assert.throws(() => isEqualErr(new Error(), null as unknown as Error), TypeError)
        assert.throws(() => isEqualErr(new Error(), 'error' as unknown as Error), TypeError)
    })
})
