...
import { isEqualMap } from '../src/index.ts'

describe('isEqualMap()', () => {
    it('returns true for the same Map instance', () => {
        const map = new Map([['a', 1]])
        assert.strictEqual(isEqualMap(map, map), true)
    })

    it('returns true for two empty maps', () => {
        assert.strictEqual(isEqualMap(new Map(), new Map()), true)
    })
    ...
    it('returns true for two maps with the same primitive key-value pairs', () => {
        ...
        assert.strictEqual(isEqualMap(map1, map2), true)
    })
    ...
    it('returns false for maps with different sizes', () => {
        ...
        assert.strictEqual(isEqualMap(map1, map2), false)
        assert.strictEqual(isEqualMap(map2, map1), false)
    })
    ...
    it('returns false for maps with different object references as values', () => {
        ...
        assert.strictEqual(isEqualMap(map1, map2), false) // strict equality fails for objects
    })

    it('returns false if the first argument is not a Map', () => {
        assert.strictEqual(isEqualMap({}, new Map()), false)
        assert.strictEqual(isEqualMap(null, new Map()), false)
        assert.strictEqual(isEqualMap(new Set(), new Map()), false)
    })

    it('throws a TypeError if the reference value is not a Map', () => {
        assert.throws(() => isEqualMap(new Map(), null as unknown as Map<any, any>), TypeError)
        assert.throws(() => isEqualMap(new Map(), {} as unknown as Map<any, any>), TypeError)
    })
})
